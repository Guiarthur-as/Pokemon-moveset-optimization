...
def run_move(B:Battle, user:Pokemon, move:dex.Move, target:Pokemon) -> None:
    ...
    if user.fainted and user.species != 'infernape':
    ...
    for i in range(number_hits):
        ...
        if B.debug:
            print(f'{user.species} did {dmg} damage to {target.species}')
        # do the damage
...
def calc_damage(B:Battle, user:Pokemon, move:dex.Move, target:Pokemon) -> int:
    ...
    if crit:
        modifier *= 1.5
    if B.debug:
        print('modifier crit: ' + str(modifier))
    # RANDOMNESS
    if B.rng:
        modifier *= random.uniform(0.85, 1.0)
    if B.debug:
        print('modifier rng: ' + str(modifier))

    # STAB (same type attack bonus)
    if move.type in user.types:
        modifier *= 1.5
    if B.debug:
        print('modifier stab: ' + str(modifier))

    
    # TYPE EFFECTIVENESS
    type_modifier = 1
    print('target types: ' + str(target.types))
    for each in target.types:
        for tp, value in typechart[each]:
            if move.type == tp:
                type_effect = value
                break
        """ type_effect = dex.typechart_dex[each].damage_taken[move.type] """
        if B.debug:
            print('type_effect: ' + str(type_effect))
        type_modifier *= type_effect
        if B.debug:
            print('type_modifier: ' + str(type_modifier))
        modifier *= type_effect
    if B.debug:
        print('modifier type: ' + str(modifier))
    ...
    if B.weather in ['rain', 'heavy_rain']:
        ...
        if move.type == 'Water':
            ...
            modifier *= 1.5
    if B.debug:
        print('modifier weather: ' + str(modifier))
    ...
    if user.status == 'brn' and \
        ...
        modifier *= 0.5
    if B.debug:
        print('modifier burn: ' + str(modifier))
    ...
    if 'minimize' in target.volatile_statuses:
        if move.id in ['dragonrush', 'bodyslam', 'heatcrash', 'heavyslam', 'phantomforce', 'shadowforce', 'stomp']:
            modifier *= 2
    if B.debug:
        print('modifier minimize: ' + str(modifier))
    ...
    if user.ability == 'sniper' and crit:
        ...
        if type_modifier < 1:
            modifier *= 2
    if B.debug:
        print('modifier abilities: ' + str(modifier))
    ...
    if target.item in list(dex.type_resist_berries.keys()):
        if dex.type_resist_berries[target.item] == move.type:
            if type_modifier > 1:
                modifier *= 0.5
    if B.debug:
        print('modifier items: ' + str(modifier))
...
def update_move_before_running(B:Battle, user:Pokemon, move:dex.Move, target:Pokemon) -> dex.Move:
    ...
    elif move.id == 'dreameater' and target.status != 'slp':
        move = move._replace(base_power = 0)
