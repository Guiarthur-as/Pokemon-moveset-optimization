...
class Pokemon:
    ...
    def __post_init__(self, poke, packed):
        ...
        self.types = pypoke.get(name=self.species).types     #dex.pokedex[self.species].types
        self.types = [t.capitalize() for t in self.types if t]
...
def dict_to_team_set(in_team : List) -> List[PokemonSet]:
    ...
    for in_poke in in_team:
        ...
        ivs = in_poke['ivs']
        level = in_poke['level']
        poke = PokemonSet(name, species, item, ability, moves, nature, evs, gender='', ivs=ivs, level=level)
        team.append(poke)
    return team
