...
def do_turn(B:Battle) -> None:
    ...
    if B.debug:
        ...
        print(B.p1.choice)
        print(B.p2.choice)
        print(f'poke 1 level: {B.p1.active_pokemon[0].level}')
        print('poke 1 HP: ' + str(B.p1.active_pokemon[0].hp))
        print(f'poke 1 {B.p1.active_pokemon[0].stats}')
        print(f'poke 1  Boosts:{B.p1.active_pokemon[0].boosts}')
        print(f'poke 2 level: {B.p2.active_pokemon[0].level}')
        print('poke 2 HP: ' + str(B.p2.active_pokemon[0].hp))
    ...
    for pokemon in get_active_pokemon(B):
        if pokemon.fainted and pokemon.species != 'infernape':
            B.pseudo_turn = True
            
    if B.pseudo_turn:
        # check player 1's pokemon
        for i in range(len(B.p1.active_pokemon)):
            if B.p1.active_pokemon[i].fainted and B.p1.active_pokemon[i].species != 'infernape':
                B.p1.request = 'switch'
            else:
                B.p1.request = 'pass'
