...
def generate_giovanni_team():
    team = []
    pokemons = {
        'rhyhorn': (['rockblast', 'scaryface', 'takedown', 'earthquake'], 42),
        'dugtrio': (['sandtomb', 'slash', 'mudslap', 'earthquake'], 42),
        'nidoqueen': (['bodyslam', 'poisonsting', 'earthquake', 'doublekick'], 44),
        'nidoking': (['thrash', 'poisonsting', 'earthquake', 'doublekick'], 45),
        'rhydon': (['rockblast', 'scaryface', 'takedown', 'earthquake'], 50)
    }
    

    for poke, (moves, level) in pokemons.items():
        pokemon = {}
        pokemon['species'] = poke
        pokemon['moves'] = moves
        pokemon['item'] = 'abomasite'
...
def generate_pokemon(move1, move2, move3, move4, level):
    team = []
    pokemon = {}
    pokemon['species'] = 'infernape'
    moves = list(dex.simple_learnsets[pokemon['species']])
    pokemon['moves'] = []
    pokemon['moves'].append(moves[move1])
    pokemon['moves'].append(moves[move2])
    pokemon['moves'].append(moves[move3])
    pokemon['moves'].append(moves[move4])
    pokemon['item'] = 'abomasite'
    pokemon['nature'] = 'hardy'
    abilities = [re.sub(r'\W+', '', ability.lower()) for ability in list(filter(None.__ne__, list(dex.pokedex[pokemon['species']].abilities)))]
    r = random.randint(0,len(abilities)-1)
